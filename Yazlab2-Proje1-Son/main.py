...
def synonymsWords2(url): #Verilen kelimenin eş anlamlı kelimelerini ekrana yazdırmak için oluşturuldu.
        ...
        for i in range(0,len(keep_keyWords_url)):
            ...
            synonymArray =  [] #Arayüze yazırmak için oluşturuldu
...
def  page4_screen(url1,url2):
        ...
        sumAll = 0
        sumSame = 0
        ortaklar = [] #herikisinde de olan ortaklar
        
        a=0
        ortaklarAll = [] #ekrana yazdırmak için oluşturuldu
        ...
        for j in range(0,len(ortaklar)):
            for k in range(0,len(url2_allWord)):
                if (ortaklar[j] == url2_allWord[k]):
                    ortaklarS.append("%s%s%s%s%s%s"%("Anahtar Kelime","[",ortaklar[j],"]","Geçme Sayısı:",url2_allWCount[k]))
...
def page3_functions(url1, url2):
    ...
    if(sumAll != 0):
       ...
       return float(a) #url1_keepWords,url2_KeepWords
    else:
      return 0 
...
def page4_functions(url):
        ...
        all_offSkor = [] 
        urlSet = ["https://www.ox.ac.uk/",
                "https://www.ox.ac.uk/research/research-impact"]
        ...
        ortaklarS = list() #arayüze yazdırmak için oluşturuldu
        ...
        for j in range(0,len(urlSet)):
                ...
                ortaklar = "%s%s%s"%(j+1,". URL :  ",page4_screen(url,urlSet[j]))
                ortaklarS.append(ortaklar)
                ...
                for i in range(0,3):
                        ...
                        ortaklar = "%s%s%s%s%s"%(j+1,". Url -> ",i+1,". alt link :  ",page4_screen(url,realLink[i]))
                        ortaklarS.append(ortaklar)
                        ...
                        if (len(realLink) == 0): #eğer alt link yok ise
                                ...
                                for k in range(0,3):
                                        ...
                                        ortaklar = "%s%s%s%s%s%s%s"%(j+1,". Url ->",i+1,". Alt link ->", k+1, ". Alt link ",page4_screen(url,realLink2[k]))
                                        ortaklarS.append(ortaklar)
                                        if (len(realLink2) == 0):
        ...
        return ortaklarS,all_offSkor,skor1_All,skor2_All,skor3_All
...
def page5_functions(url):
        ...
        for j in range(0,len(urlSet)):
                ...
                skor1.append(page5_Score(url,urlSet[j]))
                skor1_S = "%s%s%s"%(j+1,". URL : % ",result)
                skor1_All.append(skor1_S)
                all_offSkor.append(page5_Score(url,urlSet[j]))
                links = soup.find_all("a")
                symbols = [] #["#","None"]
                clearLink = []
                for link in links:
                ...
                for i in range(0,1):
                        ...
                        skor2.append(page5_Score(url,realLink[i]))
                        skor2_S = "%s%s%s%s%s"%(j+1,". Url -> ",i+1,". alt link : % ",result2)
                        skor2_All.append(skor2_S)
                        all_offSkor.append(page5_Score(url,realLink[i]))
                        ...
                        if (len(realLink) == 0):
                                ...
                                for k in range(0,1):
                                        ...
                                        skor3.append(page5_Score(url,realLink2[k]))
                                        skor3_S = "%s%s%s%s%s%s%s%s"%(j+1,". Url ->",i+1,". Alt link ->", k+1, ". Alt link",": %",result3)
                                        skor3_All.append(skor3_S)
                                        all_offSkor.append(page5_Score(url,realLink2[k]))
...
def  siteIndexing():
    if request.method == "POST":
        url1 = request.form.get("url1")
        ortaklarS , page4_function_run , skor1_All , skor2_All,skor3_All = page4_functions(url1)
        return render_template("page4.html",ortaklarS=ortaklarS,page4_function_run = page4_function_run , skor1_All = skor1_All, skor2_All = skor2_All, skor3_All = skor3_All)
    else:
        return render_template("")
